...
#include "imgui.h"
void MusicPlayUI::RenderUI(std::string path) {
    // Khởi tạo cửa sổ UI cho music player
    ImGui::Begin("Music Player");
    FileService file(path);
    std::vector<Song> mediaFiles = file.loadPlaylist();
    ...
    if(ImGui::Button("Play")){
        audioService.Play("C:\\Users\\Admin\\Downloads\\abc.mp3");
    }
    ImGui::SameLine();
    ...
    float volume = audioService.GetVolume();
